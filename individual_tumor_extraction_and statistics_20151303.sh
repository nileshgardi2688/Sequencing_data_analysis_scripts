#####################Fill all blank spaces with 0
perl -pe 's/\t(?=\t|$)/\tNULLL/g' "/media/LUN1/NIlesh_analysis/Exome/Recurrence_analysis_exome/Oncotator_output_Recurrent_Map_cos_snp_mylab.maf" > "/media/LUN1/NIlesh_analysis/Exome/Recurrence_analysis_exome/corrected_Oncotator_output_Recurrent_Map_cos_snp_mylab.maf"

sed -i 's/ /_/g' "/media/LUN1/NIlesh_analysis/Exome/Recurrence_analysis_exome/corrected_Oncotator_output_Recurrent_Map_cos_snp_mylab.maf"

# Extracting individual tumor data from recurrent file
mkdir /media/LUN1/NIlesh_analysis/Exome/Individual_tumor/
sample_list=($(cat /media/LUN1/NIlesh_analysis/Exome/Recurrence_analysis_exome/corrected_Oncotator_output_Recurrent_Map_cos_snp_mylab.maf | cut -f 170 | egrep -v '\,' |sort |uniq))
...
do
 ...
 do 
    ...
    ( IFS=$'\t'; echo ${element[@]} >> /media/LUN1/NIlesh_analysis/Exome/Individual_tumor/$i".txt")
    fi
 done < /media/LUN1/NIlesh_analysis/Exome/Recurrence_analysis_exome/corrected_Oncotator_output_Recurrent_Map_cos_snp_mylab.maf
done

sed -i 's/ /\t/g' /media/LUN1/NIlesh_analysis/Exome/Individual_tumor/*.txt

# Counting cosmic, dbsnp, cosmic+dbsnp, novel entries for individual tumor

for f in $(find /media/LUN1/NIlesh_analysis/Exome/Individual_tumor/* -name "*.txt" -type f ); 
do
  sample=$(basename $f);
	sample_name="${sample%.*}"
awk -v file="$sample_name" -F'\t' 'BEGIN{novel=0; cosmic=0; dbsnp=0; mylab=0; cosdb=0; cosmy=0; dbmy=0; cosdbmy=0} 
    ...
    }' < $f >> /media/LUN1/NIlesh_analysis/Exome/Individual_tumor/COSMIC_DBSNP_INFO.info
done
...
awk -v file="$sample_name" -F'\t' 'BEGIN{missense=0; nonsense=0; nonstop=0; splice=0; threeutr=0; fiveflank=0; fiveutr=0; igr=0; intron=0; silent=0; DenovoStartInFrame=0; DenovoStartOutOfFrame=0; FrameShiftDel=0; FrameShiftIns=0; InFrameDel=0; InFrameIns=0; lincRNA=0; RNA=0; StartCodonDel=0; StartCodonIns=0; StartCodonSNP=0; StopCodonDel=0; StopCodonIns=0;} 
    ...
    }' < $f >> /media/LUN1/NIlesh_analysis/Exome/Individual_tumor/all_mutations.info
done

## Extracting Cosmic and novel information for individual tumor
mkdir /media/LUN1/NIlesh_analysis/Exome/Individual_tumor/Novel_cosmic/
for f in $(find /media/LUN1/NIlesh_analysis/Exome/Individual_tumor/* -name "*.txt" -type f ); 
do
  sample=$(basename $f);
	sample_name="${sample%.*}"
awk -F'\t'  '{if($121 ~ /-/ && $257 ~ /-/ && $168 ~ /-/) print $0; 
             ...
             }' < $f >> /media/LUN1/NIlesh_analysis/Exome/Individual_tumor/Novel_cosmic/$sample_name"_novel_cosmic.txt"
done


###################################### Into cosmic and novel we make table of all types of mutations calculations
for f in $(find /media/LUN1/NIlesh_analysis/Exome/Individual_tumor/Novel_cosmic/* -name "*.txt" -type f ); 
do
  sample=$(basename $f);
	sample_name="${sample%.*}"
awk -v file="$sample_name" -F'\t' 'BEGIN{missense=0; nonsense=0; nonstop=0; splice=0; threeutr=0; fiveflank=0; fiveutr=0; igr=0; intron=0; silent=0; DenovoStartInFrame=0; DenovoStartOutOfFrame=0; FrameShiftDel=0; FrameShiftIns=0; InFrameDel=0; InFrameIns=0; lincRNA=0; RNA=0; StartCodonDel=0; StartCodonIns=0; StartCodonSNP=0; StopCodonDel=0; StopCodonIns=0;} 
    ...
    print "Silent entries=""\t"silent;
    print "Splice entries=""\t"splice"\n"
    }' < $f >> /media/LUN1/NIlesh_analysis/Exome/Individual_tumor/Novel_cosmic/Cosmic_novel_all_mutations.info
done
